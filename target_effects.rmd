    ...
    if(cur_trial == 1){
      ...
      for (j in (cur_trial-1):last_back){
        ...
        # } else {
          ...
          # rate = rate + (.5^(nb))*data[data$subj == subject & data$trial_test == j,'rewarded']
...
filt_data = c()
filt_data = filter(test,trial_in_block>0)
...
```{r, echo = FALSE}
lme_test = list()
lme_test[['r_prob']][['abs']][['peak_vel']] <- cftest(lmer(peak_vel ~ factor(target) + r_prob + (1|subj), data = filt_data))
lme_test_t14 = list()
lme_test_t14[['r_prob']][['abs']][['peak_vel']] <- cftest(lmer(peak_vel ~ factor(target) + r_prob + (1|subj), 
                                                           data = rbind(filter(filt_data,r_prob==1),
                                                                        filter(filt_data,r_prob==0))))

# summary(lme(peak_vel~r_prob+one_back_rate+as.factor(target), random =~1|subj, data=test))
 
#============= No Facet Abs
a1 = aggregate(peak_vel ~ r_prob + one_back_rate + subj ,filt_data,mean)
a  = aggregate(peak_vel ~ r_prob + one_back_rate ,a1,mean)
b  = aggregate(peak_vel ~ r_prob + one_back_rate ,a1,sd)/sqrt(max(a1$subj))#/sqrt(length(filt_data[,1]))
c  = cbind(a,b$peak_vel)
colnames(c) = c('r_prob','one_back_rate','peak_vel','peak_vel_se')

ggplot(data = c,
       aes(x = factor(r_prob),
           y = peak_vel,
           color = factor(one_back_rate),
           group = factor(one_back_rate)))+
  geom_point(data = c,
             aes(x = factor(r_prob),
                 y = peak_vel,
                 color = factor(one_back_rate),
...
b  = aggregate(peak_vel ~ r_prob + target,a1,sd)/sqrt(max(a1$subj))#/sqrt(length(filt_data[,1]))
c  = cbind(a,b$peak_vel)
...
#============= Facet Abs
a1 = aggregate(peak_vel ~ r_prob + one_back_rate + subj + target,filt_data,mean)
a  = aggregate(peak_vel ~ r_prob + one_back_rate + target,a1,mean)
b  = aggregate(peak_vel ~ r_prob + one_back_rate + target,a1,sd)/sqrt(max(a1$subj))#/sqrt(length(filt_data[,1]))
c  = cbind(a,b$peak_vel)
colnames(c) = c('r_prob','one_back_rate','target','peak_vel','peak_vel_se')
...
```{r, echo = FALSE}
lme_test[['r_prob']][['norm']][['peak_vel_norm']] <- cftest(lmer(peak_vel_norm ~ factor(target) + r_prob + (1|subj), data = filt_data)) 
lme_test_t14[['r_prob']][['norm']][['peak_vel_norm']] <- cftest(lmer(peak_vel_norm ~ factor(target) + r_prob + (1|subj), 
                                                                 data = rbind(filter(filt_data,r_prob==1),
                                                                              filter(filt_data,r_prob==0))))
...
b  = aggregate(peak_vel_norm ~ r_prob ,a1,sd)/sqrt(max(a1$subj))#/sqrt(length(filt_data[,1]))
...
#============= Facet Abs
a1 = aggregate(peak_vel_norm ~ r_prob + one_back_rate + subj + target,filt_data,mean)
a  = aggregate(peak_vel_norm ~ r_prob + one_back_rate + target,a1,mean)
b  = aggregate(peak_vel_norm ~ r_prob + one_back_rate + target,a1,sd)/sqrt(max(a1$subj))#/sqrt(length(filt_data[,1]))
c  = cbind(a,b$peak_vel)
colnames(c) = c('r_prob','one_back_rate','target','peak_vel','peak_vel_se')
...
b  = aggregate(peak_vel_norm ~ r_prob + target,a1,sd)/sqrt(max(a1$subj))#/sqrt(length(filt_data[,1]))
c  = cbind(a,b$peak_vel_norm)
...
lme_test[['r_prob']][['diff']][['peak_vel_diff']] <- cftest(lmer(peak_vel_diff ~ factor(target) + r_prob + (1|subj), data = filt_data))
lme_test_t14[['r_prob']][['diff']][['peak_vel_diff']] <- cftest(lmer(peak_vel_diff ~ factor(target) + r_prob + (1|subj), 
                                                                 data = rbind(filter(filt_data,r_prob==1),
                                                                              filter(filt_data,r_prob==0))))
...
b  = aggregate(peak_vel_diff ~ r_prob ,a1,sd)/sqrt(max(a1$subj))#/sqrt(length(filt_data[,1]))
...
b  = aggregate(peak_vel_diff ~ r_prob + target,a1,sd)/sqrt(max(a1$subj))#/sqrt(length(filt_data[,1]))
c  = cbind(a,b$peak_vel_diff)
...
# #========
# 
# ggplot()+geom_violin(data = filt_data,
#                      aes(x = factor(r_prob),
#                          y = peak_vel_diff))+
#   facet_wrap(.~target)
