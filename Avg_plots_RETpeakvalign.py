...
def filler_func(data, time_points):
    data = np.pad(data, (0, max(time_points)-len(data)+1000), 'constant')
...
for s, subj in enumerate(cfdata):
    for t, trial in enumerate(subj.items()):
        if max(trial[1]['P']) > .08 and len(trial[1]['P'])-trial[1]['vigor']['idx']['peakv']>500:
            ...
            if np.argmin(trial[1]['rad_v'])>trial[1]['vigor']['idx']['peakv']:
                min_thin_retpeakv = min(min_thin_retpeakv,np.argmin(trial[1]['rad_v']))
...
test_var_retpeakv_align= [item+'_retpeakv_align' for item in test_var_abs]
# test_var_names = ['P',
#         'px_abs',
#         'py_abs',
#         'v_sign',
...
for item in test_var_peakv_align:
    exec(item+'= []')
for item in test_var_retpeakv_align:
    exec(item+'= []')
...
for s, subj in enumerate(cfdata):
    ...
    for t, trial in enumerate(subj.items()):
        ...
        # if max(trial[1]['P']) > .08 and check2 and check3 and check4:
        if max(trial[1]['P']) > .08 and check3:
            block = 0
            ...
            for it, item in enumerate(test_var_names):
                ...
                # data = filler_func(trial[1][str(item)], RETpeak_v_align)
                # data = np.array(data)*flipit
                # data = np.array(np.pad(trial[1][str(item)], (0, 1000), 'constant')[RETpeak_v_align])*flipit
                data = trial[1][str(item)]
                nans = np.zeros(5000) + np.nan
                data = np.append(data,nans)
                data = data[RETpeak_v_align]
                # max_diff.append(max(abs(np.diff(data))))
                exec(test_var_retpeakv_align[it]+'.append(data)')
...
for probability_method in ['prior_RWD','RWD','r_prob','RPE','prior_RPE']:#,'r_prob','RPE','prior_RPE']:
    ...
    # probability_method = 'RPE'
    # independent_var = RPE
    # v_subj_avg = []
    # c_subj_avg = []
    # for s in range(len(cfdata)):
    #     # subj_trace = v_peakv_align[subject == s]
    #     subj_trace = v_rad_retpeakv_align[subject == s]
    ...
    # test_var_label = 'Velocity (m/s)'
    # test_var = np.array(v_subj_avg)
    # c_subj_avg = np.array(c_subj_avg)
    # norm_lab = 'RETURN Peak Velocity Align'
    ...
    #     lines.append(plt.plot(np.nanmean(test_var[c_subj_avg==cond]-mean_0,0),
    #                                     color = viridis_big.colors[k]))
    ...
    # axes.set_xlabel('Time (s), Aligned to RET Peak V')#%%

    # fig_str = ('RadVel_avg_'+probability_method+
    #     '_SMOLGROUP_retpeakvalign'+'.pdf')
    ...
    # test_var = []
    # exec('test_var = ' + test_var_retpeakv_align[test_var_num])
    # norm_lab = 'RET Peak Velocity Align'
    ...
    #     #             color = viridis_big.colors[k]))
    #     lines.append(plt.plot(np.nanmean(test_var[independent_var==cond]-mean_0,0),
    #                                     color = viridis_big.colors[k]))
    ...
    # fig_str = (labels2[test_var_num][0:3]+'_'+'avg_plot_delta'+probability_method+'_'+'.pdf')
    # fig_str = ('RadVel_avgdelta_'+probability_method+
    #     '_retpeakvalign'+'.pdf')
    # fig.savefig(fig_str,format = 'pdf')
    ...
    # fig_str = ('RadVel_avg_'+probability_method+
    #     '_retpeakvalign'+'.pdf')
    fig_str = ('avg_plot_'+labels2[test_var_num][0:3]+'_'+probability_method+'_RETpeakvalign.pdf')
    fig.savefig(fig_str,format = 'pdf')
